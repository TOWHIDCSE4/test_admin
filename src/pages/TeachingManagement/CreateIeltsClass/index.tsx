...
import { EnumCourseType, IOrderedPackage } from 'types'
...
const CreateIeltsClass: FC = () => {
                ...
                .then((res) => {
                    ...
                    if (_.isEmpty(res.data))
                        notification.error({
                            message: 'Error',
                            description:
                                'Student has no package active. Can not create anything'
                        })
                })
                .catch((err) => {
                    notification.error({
    ...
    const getCourseByPackageId = useCallback(
        (query: {
            ...
            const { filter, courses } = values
            setValues({ isLoading: true })
            CourseAPI.getCourseByPackageId(query.package_id, {
                ...query,
                course_type: EnumCourseType.IELTS
            })
                .then((res) => {
            ...
            if (!_.isEmpty(changedValues)) {
                ...
                if (_.has(changedValues, 'ordered_package_id')) {
                    ...
                    if (temp) {
                        ...
                        form.resetFields(['course', 'unit_id', 'start_time'])
    ...
    const saveIeltsClass = (valuesForm) => {
        setValues({ isLoading: true })
        BookingAPI.createIeltsBooking({
            ...valuesForm,
            ...
            .then((res) => {
                ...
                form.resetFields(['start_time', 'note'])
            })
            .catch((err) => {
                notification.error({
                    message: 'Error',
                ...
                <Card title='Create Ielts Class'>
                    ...
                    >
                        ...
                        >
                            <Select
                                style={{ width: '100%' }}
                                onChange={(val) => setValues({ status: val })}
                            >
                                <Option
                                    key='completed'
                                    value={ENUM_BOOKING_STATUS.COMPLETED}
                                >
                                    COMPLETED
                                </Option>
                                <Option
                                    key='upcoming'
                                    value={ENUM_BOOKING_STATUS.UPCOMING}
                                >
                                    UPCOMING
                                </Option>
                                <Option
                                    key='student_absent'
                                    value={ENUM_BOOKING_STATUS.STUDENT_ABSENT}
                                >
                                    STUDENT_ABSENT
                                </Option>
                            </Select>
...
export default CreateIeltsClass
